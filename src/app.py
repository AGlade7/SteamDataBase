...
def add_gpc():
    try:
        ...
        with get_db() as cursor:
            cursor.execute(f"SELECT add_gpc({p_gpc_id}, '{p_gpc_name}', '{p_email_id}')")
...
def add_game():
    try:
        ...
        with get_db() as cursor:
            cursor.execute(
                f"SELECT add_game({p_game_id},'{p_game_name}', {p_price},{p_gpc_id},'{p_game_release_date}', {p_age_limit});"
            )
