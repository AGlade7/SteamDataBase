...
@app.get("/get-games-by-genre-region-and-age/{genre_name}/{region_name}/{user_age}", response_model = list, status.HTTP_200_OK)
def get_games_by_genre_region_and_age(genre_name: str, region_name: str, user_age: int):
    try:
        with get_db() as db:
            db.callproc("get_games_by_genre_region_and_age", [genre_name, region_name, user_age])
            results = db.fetchall()
        ...
        raise HTTPException(
            ...
            detail=f"Error calling stored procedure get_games_by_genre_region_and_age: {e}",
        )
...
@app.get("/get-reviews-for-game/{game_id}", methods=["POST"])
def get_reviews_for_game(game_id: int):
    try:
        with get_db() as db:
            db.callproc("get_reviews_for_game", [game_id])
            results = db.fetchall()
